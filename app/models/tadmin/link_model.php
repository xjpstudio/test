...
class Link_model extends Model
{
	...
	function add_record($f_path)
	{
		...
		$this->db->insert("shop_link",$data);
		return TRUE;
	}
	
	function get_record($id)
	{
		$query=$this->db->get_where("shop_link",array('id'=>$id));
		if($query->num_rows()>0)
		{
	...
	function sort_record()
	{
		...
		foreach($rd_id as $row)
		{
			...
			$this->db->update("shop_link",array('seqorder'=>$sort_value),array('id'=>$row));
		}
		return TRUE;
	}
	...
	function save_record($f_path)
	{
		...
		$this->db->update("shop_link",$data,array('id'=>$rd_id));
		return TRUE;
	}
