...
class Flash_model extends Model
{
	...
	function add_record($f_path)
	{
		...
		$this->db->insert("shop_flash",$data);
		return TRUE;
	}
	
	function get_record($id)
	{
		$query=$this->db->get_where("shop_flash",array('id'=>$id));
		if($query->num_rows()>0)
		{
	...
	function sort_record()
	{
		...
		foreach($rd_id as $row)
		{
			...
			$this->db->update("shop_flash",array('seqorder'=>$sort_value),array('id'=>$row));
		}
		return TRUE;
	}
	...
	function save_record($f_path)
	{
		...
		$this->db->update("shop_flash",$data,array('id'=>$rd_id));
		return TRUE;
	}
