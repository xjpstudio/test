...
<table width="100%"  border="0" cellpadding="3" cellspacing="0" >
  <tr>
      ...
      <td width="84%"><input name="num_iid" id="num_iid" type="text" size="20" maxlength="20" dataType="Custom" msg="ID格式有误" regexp="^[1-9]+\d*$" value="<?php echo $item['num_iid'];?>" /> &nbsp; <input type="button" onclick="get_data()" value="获取数据" class="button-style" /> <br>
      例如：http://detail.tmall.com/item.htm?id=37497291598 链接中的<font color="#FF0000">39305121854</font> 就是商品ID
      </td>
    </tr>
</table>
<div class="ui-tx-block-title">2、商品基本信息</div>
  <table width="100%"  border="0" cellpadding="3" cellspacing="0" >
    <tr>
      ...
      <td width="84%"><input name="title" value="<?php echo $item['title'];?>" type="text" size="80" maxlength="100" dataType="Require" msg="该项必须填写" /></td>
    </tr>
    <tr>
      <td valign="top"><font color="#FF0000">*</font>商品链接：</td>
      <td><input name="click_url" id="click_url" value="<?php echo $item['detail_url'];?>" type="text" size="80" datatype="Require" msg="该项必须填写" /> <input type="button" onclick="get_data1()" value="获取数据" class="button-style" /> <br>请填写淘宝或天猫商品详细页地址，例如：http://detail.tmall.com/item.htm?id=37497291598</td>
    </tr>
    <tr>
      <td width="16%" valign="top"><font color="#FF0000">*</font>零售价(￥)：</td>
      <td width="84%"><input name="shop_price" type="text" size="10" maxlength="10" dataType="Currency" msg="价格格式有误" value="<?php echo $item['shop_price'];?>" /></td>
    </tr>
    <tr>
      <td width="16%" valign="top"><font color="#FF0000">*</font>折扣价(￥)：</td>
      <td width="84%"><input name="dc_price" type="text" size="10" maxlength="10" dataType="Currency" msg="价格格式有误" value="<?php echo $item['dc_price'];?>" /> <font color="#FF0000">如果没有折扣价，请填写数字0</font></td>
    </tr>
    <tr>
          <td><font color="#FF0000">*</font>销量：</td>
          <td><input name="volume" type="text" size="10" value="<?php echo $item['volume'];?>" maxlength="10" dataType="Int" msg="销量格式有误" /></td>
        </tr>
        <tr>
      ...
      <td width="84%"><input name="nick" type="text" size="30" maxlength="100"  value="<?php echo $item['nick'];?>" />  <a href="http://www.tianxianet.com/help/679.html" target="_blank" style="color:#06F">查看掌柜昵称获取教程</a></td>
    </tr>
    <tr>
      ...
      <img src="<?php echo $v;?>_60x60.jpg" height="60" width="60" /><br>
      <?php $pic_url = $v;
	  break;}}?>
      <input type="text" name="pic_path" id="pic_path" size="25" maxlength="300" value="<?php echo $pic_url;?>" />
        ...
        <?php if($pic_url){?>
        <input type="hidden" name="small_pic_path" value="<?php echo $pic_url.'_230x230.jpg';?>" />
        <?php }?>
        <br />
    ...
    <tr>
      ...
      <ul>
      <?php $i = 1;if(count($item['pic_url']) > 1){ foreach($item['pic_url'] as $v){
		  if($i > 1){
		  ?>
          <li><?php if($i == 2){?><a href="javascript:extend_upload_input()" title="增加一张图片">[+]</a><?php }else{?><span style="cursor:pointer; color:#FF0000" title="删除" onclick="remove_ob(this)">[-]</span><?php }?> <input type="text" name="pic_path<?php echo ($i-1);?>" maxlength="300" size="25" value="<?php echo $v;?>" /> <input name="more_pic<?php echo ($i-1);?>" type="file" size="15" dataType="Filter" accept="<?php echo str_replace("|",",",UP_IMAGES_EXT);?>" require="false" msg="图片格式必须是：<?php echo UP_IMAGES_EXT;?>"><input type="hidden" value="<?php echo ($i-1);?>" name="more_pic_id[]" />
          </li>
       <?php }
	   $i++;}}else{?>
    ...
    <tr>
      ...
      <td><input name="keyword" type="text" value="<?php echo $item['title'];?>" size="80" maxlength="100" /><br>
        一般不超过100个字符</td>
    </tr>
    <tr>
      <td valign="top">meta描述：</td>
      <td><textarea name="description" cols="80" rows="3" dataType="Limit" max="200" msg="字数不超过200" require="false"><?php echo $item['title'];?></textarea><br>
        一般不超过200个字符</td>
    </tr>
    <tr>
    ...
    <tr>
      <td colspan="2" style="padding-bottom:5px"><div class="ui-tx-block-title">3、商品描述</div>
       <textarea name="content" id="content" style="width:780px; height:450px;visibility:hidden;"><?php echo htmlspecialchars($item['content']);?></textarea></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
...
var global_loop=$(".up_pic_form li").length;
