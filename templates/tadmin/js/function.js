...
//转移分类
function to_catalog_onChange()
{
	var selected_text=$("#action_url option:selected").text();
	if(selected_text=='转移到分类'||selected_text=='复制到'||selected_text=='剪切到')
	{
		$("#to_catalog_span").show();
	}
	else
	{
		$("#to_catalog_span").hide();
	}
}
//拷贝文本
...
//切换状态
function ajax_toggle(ob,id,url)
{
	...
	$.ajax({
		 ...
		 success: function(msg){ 
		 	if(msg==1) {
				var img_src=$(ob).attr("src");
				if(img_src.indexOf("no.gif")>0)
				{
					$(ob).attr("src",img_src.replace("no.gif","yes.gif"));
				}
				else
				{
					$(ob).attr("src",img_src.replace("yes.gif","no.gif"));
				}
			}
			hide_message();
		 }
...
function make_html(v)
{
	show_message('数据处理中...',false);
	$.ajax({
		url:js_root_path+"make_html/make_sg_news",
		data:"id="+v,
		type:"POST",
		dataType:"json",
		success:function(msg)
		{
			...
			{
				$('#ui-tx-mark-message').html('生成完毕');
				setTimeout(function(){hide_message();},2000);
			}
			else
			{
				$('#ui-tx-mark-message').html('生成失败，请重新提交');
				setTimeout(function(){hide_message();},2000);
			}
		}
	});
