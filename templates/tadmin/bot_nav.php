...
<table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" >
  <tr>
    ...
    <td width="97%" align="left" >您现在的位置：网站底部导航</td>
  </tr>
</table>
...
<form name="addForm">
  <table width="100%"  border="0" cellpadding="3" cellspacing="0" style="margin:10px 0">
    <tr>
      ...
      <td width="594"><input type="text" size="50" maxlength="300" name="url" id="url" dataType="Require" msg="该项必填" />&nbsp;
    ...
    <tr>
      ...
      <td width="594"><input name="title" id="title" type="text" size="15" dataType="Require" msg="该项必须填写" maxlength="50" /></td>
    </tr>
    <tr>
    ...
    <tr>
      <td width="119">&nbsp;</td>
      <td width="594"><input type="button" name="sbmit" onclick="submit_add_form()" value="添加到当前导航" class="button-style" style="width:100px" /></td>
    </tr>
  </table>
</form>
</fieldset>
<form method="post" action="<?php echo site_url(CTL_FOLDER.'bot_nav/save_bot_nav');?>">
  <fieldset>
  <legend>当前导航(双击可从当前导航中删除，拖动可对导航进行排序)</legend>
  <div class="nav_box">
    <ul id="cnav">
      <?php foreach($bot_nav as $v){?>
      <li><?php echo $v['title'];?>
        <input type="hidden" name="bot_nav[]" value="<?php echo $v['title'].'|'.$v['url'].'|'.$v['target'];?>" />
      </li>
      <?php }?>
    </ul>
  </div>
...
function submit_add_form()
{
	...
	if( ! checkr(str))
	{
		$('<li>'+title+'<input type="hidden" name="bot_nav[]" value="'+str+'" /></li>').appendTo('#cnav');
		theform.reset();
	}
	else
	{
